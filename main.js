...
function buildExamMap(list, mode='nat'){
  ...
  const enemSorted = order
    .filter(ex => ex.toUpperCase().startsWith('ENEM-'))
    .sort((a,b)=>{
      const pa=a.split('-');
      const pb=b.split('-');
      ...
      const restA=pa.slice(3).join('-');
      const restB=pb.slice(3).join('-');
  ...
  for(let i=0;i<order.length;i++){
    if(order[i].toUpperCase().startsWith('ENEM-')){
      order[i]=enemSorted[ei++];
    }
  }
